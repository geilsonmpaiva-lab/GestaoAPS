  ...
  return <main className="login-page">
    ...
    <section className="login-form-wrap">
      <form className="login-card" onSubmit={recovery ? updatePassword : signIn}>
        ...
        <label>{recovery ? "Nova senha" : "Senha"}<div className="login-input"><KeyRound size={16} /><input type={recovery&&showNewPassword ? "text" : "password"} required minLength={recovery ? 10 : undefined} autoComplete={recovery ? "new-password" : "current-password"} value={recovery ? newPassword : password} onChange={(event) => recovery ? setNewPassword(event.target.value) : setPassword(event.target.value)} placeholder={recovery ? "Mínimo de 10 caracteres" : "Sua senha"} />{recovery&&<button type="button" className="password-visibility" onClick={()=>setShowNewPassword((visible)=>!visible)} aria-label={showNewPassword ? "Ocultar senhas" : "Mostrar senhas"} aria-pressed={showNewPassword}>{showNewPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button>}</div></label>
