...
export async function POST(request:Request,context:{params:Promise<{id:string}>}) {
  ...
  if(!z.uuid().safeParse(id).success||!body.success) return NextResponse.json({error:"Comando de início inválido."},{status:422});
  if(isDemoMode()) return NextResponse.json({id,status:"IN_PROGRESS",version:body.data.expectedVersion+1,mode:"demo"});
  const client=await createSupabaseServerClient();
  const {data,error}=await client.rpc("start_meeting",{p_meeting_id:id,p_expected_version:body.data.expectedVersion,p_operation_id:body.data.operationId});
